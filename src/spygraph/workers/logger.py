...
def format_json_output(data: dict) -> None:
    if data.get("status") == "started":
        ...
        panel = Panel(Text("\n  API Server stopped\n", style="bold magenta"), 
                      title="API SERVER STOPPED", box=box.ROUNDED, border_style="magenta", expand=False)
        console.print(panel)
        console.print()
        ...
        content.append(f"    ◆ Browser: {fingerprint.get('browser', 'Unknown')} {fingerprint.get('browser_version', '')}\n", style="magenta") # noqa: E501
...
def process_logs(queue: Queue) -> None:
    ...
    try:
        ...
        panel = Panel(
            Text("  Shutdown complete", style="bold magenta"),
            title="LOGGER STOPPED",
            box=box.ROUNDED,
            border_style="magenta",
            expand=False
        )
